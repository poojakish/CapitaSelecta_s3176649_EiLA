...
def main():
    ...
    valdir = args.data#os.path.join(args.data, 'valid')
...
def validate_test(val_loader, model, criterion, args):
    ...
    # switch to evaluate mode
    model.eval()
    confusion_matrix = np.zeros((7, 7), dtype=np.float32)
    y_true_all = []
    y_pred_all = []
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            # Flatten targets and predictions for metric calculations
            ...
            confusion_matrix += metrics.confusion_matrix(
                target, pred, labels=[0, 1, 2, 3, 4, 5, 6]
            )
        ...
        # F1-Score (Weighted)
        f_score = metrics.f1_score(y_true_all, y_pred_all, average='weighted')

        # Weighted Accuracy (Balanced Accuracy)
        recalls_per_class = np.diag(confusion_matrix) / np.sum(confusion_matrix, axis=1)
        weighted_accuracy = np.mean(recalls_per_class)
        ...
        with open('./log/' + time_str + 'log.txt', 'a') as f:
            ...
            f.write(f' * F1-Score (Weighted): {f_score:.3f}\n')
            f.write(f' * Weighted Accuracy: {weighted_accuracy:.3f}\n')

    return acc, losses.avg, confusion_matrix_percent

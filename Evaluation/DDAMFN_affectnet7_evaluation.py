...
def parse_args():
    ...
    parser.add_argument('--aff_path', type=str, default='../gender_distribution/female/', help='AffectNet dataset path.')
...
class8_names = ['Neutral', 'Happy', 'Sad', 'Surprise', 'Fear', 'Disgust', 'Anger', 'Contempt']
...
def run_test():
    ...
    if args.num_class == 7: 
        ...
        plot_confusion_matrix(matrix, classes=class7_names, normalize=False, title= 'affectnet  Confusion Matrix (acc: %0.2f%%)' %(acc*100))
        ...
        np.set_printoptions(precision=2)
        plt.figure(figsize=(10, 8))
        plot_confusion_matrix(matrix, classes=class8_names, normalize=False, title= 'AffectNet Confusion Matrix (acc: %0.2f%%)' %(acc*100))
...
def run_test_2():
    ...
    val_dataset = datasets.ImageFolder(f'{args.aff_path}', transform=data_transforms_val)
    if args.num_class == 7:
        idx = [i for i in range(len(val_dataset)) if val_dataset.imgs[i][1] != 7]
        val_dataset = data.Subset(val_dataset, idx)
    ...
    # Compute F1-Score
    f1 = f1_score(all_targets.cpu().numpy(), all_predicted.cpu().numpy(), average='weighted')

    # Compute Weighted Accuracy
    matrix = confusion_matrix(all_targets.cpu().numpy(), all_predicted.cpu().numpy())
    per_class_accuracy = np.diag(matrix) / np.sum(matrix, axis=1)
    weighted_accuracy = np.sum(per_class_accuracy * np.sum(matrix, axis=1) / np.sum(matrix))

    print("Validation accuracy: %.4f" % acc)
    print("F1-Score (Weighted): %.4f" % f1)
    print("Weighted Accuracy: %.4f" % weighted_accuracy)

    if args.num_class == 7:
        plt.figure(figsize=(10, 8))
        plot_confusion_matrix(matrix, classes=class7_names, normalize=False,
                              title='AffectNet Confusion Matrix (Acc: %.2f%%)' % (acc * 100))
        plt.savefig(os.path.join('checkpoints', "affecnet7" + "_acc" + str(acc) + ".png"))
        plt.close()
    elif args.num_class == 8:
        plt.figure(figsize=(10, 8))
        plot_confusion_matrix(matrix, classes=class8_names, normalize=False,
                              title='AffectNet Confusion Matrix (Acc: %.2f%%)' % (acc * 100))

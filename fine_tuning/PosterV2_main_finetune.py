...
parser.add_argument('--data', type=str, default=r'/Users/poojakishore/Documents/Codes/POSTER_V2-main/split_data_new')
...
parser.add_argument('-b', '--batch-size', default=32, type=int, metavar='N') #changes default from 144 to 32
parser.add_argument('--optimizer', type=str, default="adam", help='Optimizer, adam or sgd.')

parser.add_argument('--lr', '--learning-rate', default=0.000015, type=float, metavar='LR', dest='lr') #changed the default from 0.000035 to 0.000015
...
def main():
    ...
    if args.evaluate is not None:
        ...
        validate_test(val_loader, model, criterion, args)
        return
...
def validate_test(val_loader, model, criterion, args):
    losses = AverageMeter('Loss', ':.4f')
    top1 = AverageMeter('Accuracy', ':6.3f')
    progress = ProgressMeter(len(val_loader),
                             [losses, top1],
    ...
    # switch to evaluate mode
    model.eval()
    num_classes = 7  # Assuming there are 7 classes
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            confusion_matrix += metrics.confusion_matrix(target, pred, labels=list(range(num_classes)))

            if i % args.print_freq == 0:
                progress.display(i)

        # Compute additional metrics
        # F-score and Weighted Accuracy
        ...
        f_scores = np.where(precision + recall > 0, 2 * (precision * recall) / (precision + recall), 0)
        weighted_accuracy = np.sum(tp / class_counts * class_counts) / np.sum(class_counts)

        mean_f_score = np.mean(f_scores)

        print(' **** Accuracy {top1.avg:.3f}, F-score: {f_score:.3f}, Weighted Accuracy: {weighted_acc:.3f} *** '.format(
            top1=top1, f_score=mean_f_score, weighted_acc=weighted_accuracy))
        with open('./log/' + time_str + 'log.txt', 'a') as f:
            f.write(' * Accuracy {top1.avg:.3f}, F-score: {f_score:.3f}, Weighted Accuracy: {weighted_acc:.3f}\n'.format(
                top1=top1, f_score=mean_f_score, weighted_acc=weighted_accuracy))

    return top1.avg, losses.avg, mean_f_score, weighted_accuracy, confusion_matrix
